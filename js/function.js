...
function handlePlayPage() {
    ...
    $('#show-result').on('click', function(e) {
        ...
        // ラッキーアイテムの取得（設定がONの場合のみ）
        const lucky = settings.showLuckyItem && result.luckyItem
            ? result.luckyItem
            : null;
...
function handleResultPage() {
    ...
    if (settings.showLuckyItem && data.lucky) {
        ...
        // ラッキーアイテムを表示しない場合、ラッパー要素を削除
        $('#lucky-wrapper').remove();
    }
    
    // 備考：もし結果画像用の要素がHTMLに残っている場合は削除
    $('#result-img').remove();
...
function handleHistoryPage() {
    ...
    history.forEach(item => {
        // ラッキーアイテムがあれば表示に追加
        const luckyDisplay = item.lucky ? ` (ラッキーアイテム: ${item.lucky})` : '';
